...
def qvalue(pos,vel,a):
	...
	Qvalue = QL[(pos_in_grid,vel_in_grid,a)]
	return Qvalue
...
def QLupdate(reward, act, oldp, oldv, newp, newv):
    ...
    qval = qvalue(pos_in_grid, vel_in_grid, act)
    QL[(pos_in_grid, vel_in_grid, act)] = (1-beta) * QL[(pos_in_grid, vel_in_grid, act)] + (beta)*(reward + gamma * best_new_qval) # Q-Learning update rule
	...
	for run in range(0, runs):
		...
		for trial in range(0, max_trials):
		    while True:
			    ...
			    iterations += 1
                if mygoal: break
                else: print simulatev, simulatep
